...
Mozart is pluggable for three surfaces: **ticketing**, **documentation**, and **code retrieval**. You configure them by adding stanzas to your repo's `AGENTS.md`. Mozart and his specialists read those stanzas at intake; if a stanza is missing, the corresponding behavior is skipped or falls back to a sensible default.

This file is the contract. Copy the appropriate stanza into your repo's `AGENTS.md`, fill in the values, and the plugin adapts.
...
**Where to put it:** in your repo's `AGENTS.md`, add a `## Ticketing` section.
...
**Where to put it:** in your repo's `AGENTS.md`, add a `## Documentation surfaces` section.
...
1. **Declare the tool** in your repo's `AGENTS.md` so agents know it exists and is authoritative:
...
Switching from one ticketing system to another (e.g., Plane → Linear) is just a `AGENTS.md` edit. Mozart re-resolves at the next intake. In-flight campaigns keep their original ticket; new campaigns use the new system.
