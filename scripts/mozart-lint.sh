...
# to hold: status-vs-location drift, paths-vs-checkbox external-review drift,
# duplicate stage lines, unclosed stage lists in terminal campaigns, stale
# active campaigns, stranded sibling artifacts, stale active/ path references
# inside finished state files, and active DELIVER(-family) campaigns missing
# their 2b. Constraints row.
#
...
# as Checks C/D — unlike J, which is active/-only, K/L also read finished/
...
# below — a fixture-corpus test hook; every run that sets it prints the
# override-visibility line below before any `LINT` line, so an override can
# never be silent.
#
# This port omits mozart-orchestration's Check I (missing-12b): codex has no
# stage 12b — it lacks upstream commit e9232c0 (`.mozart/` root, worktree
# isolation, the version-resolution gate) and the 12b. Ship stage that check
# gates. Porting Check I unmodified would flag every codex DELIVER campaign
...
#
# Covers both artifact roots — .mozart/ (mozart-orchestration's convention; a
# target repo may follow it even under this port) and thoughts/shared/ (this
# port's native layout) — and within each, three layouts: the current subdir
...
# (stale paths), H (stranded siblings) and J (2b) skip them by construction —
# each of those asks "is this file where its status says it should be" or
# "does this in-flight campaign conform to the current template", and a flat
# file answers neither question from its path.
#
...
# PD1/PD3/PD24. CONDUCTOR_SINCE defaults to the merge date; the fixture-hook
# override is documented above and asserted by the parity tool's `behaviour`
# subcommand.
...
# Artifact roots. A repo may have either, both, or neither.
ROOTS=()
...
if [ "${#ROOTS[@]}" -eq 0 ]; then
  echo "mozart-lint: no $ROOT/.mozart/plans (or $ROOT/thoughts/shared/plans) — nothing to lint"
  exit 2
fi
...
status_of() {
  local s
  # A combined header ("**Shape**: ... | **Status**: in-progress | **Flow**:
  # ...") puts **Status**: after other fields on the same line. Match
  # anywhere, take the value up to the next "|" (if any) or end of line.
...
flow_of() {
  # Same combined-header hazard as status_of() -- match **Flow**: anywhere
  # in the line, stop at the next "|" or end of line.
...
# Markdown table rows were split on a RAW pipe, so a `source` cell holding a
# shell pipeline shifted every later cell and the linter read the next cell
# along as `control` — an empty control parsed as filled, and the row passed
# clean. Two halves, both needed:
...
BEGIN {
  ...
  if (decisions_file != "" && decisions_file != "/dev/null") {
    while ((getline dline < decisions_file) > 0) {
      ...
      # S4 recommends "Revisit trigger", but "Revisit when" is an accepted
...
# A combined header ("**Shape**: ... | **Tier**: ... | **Flow**: OPERATE-FULL")
# puts **Flow**: after other fields on the same line. Match anywhere, take
# the value up to the next "|" (if the line has more fields after it) or end
# of line.
raw ~ /\*\*Flow\*\*:/ {
  fv = raw
  sub(/^.*\*\*Flow\*\*:[ \t]*/, "", fv)
...
in_change && raw ~ /^\|/ {
  ...
  # Same width rule as the conductor table -- a rollback command with an
  ...
  # Name the specific failing reason (which token, or the field count)
...
in_findings && raw ~ /^\|/ {
  if (raw ~ /\| *id *\|/) next
  if (raw ~ /^\|[- |]+\|$/) next
  # Residual, stated plainly: this table is read POSITIONALLY (no header
...
END {
  ...
  # section's CONTENT must ask. decision-trigger used to ask post_by_date, so
  # a campaign with a conductor record and a pre-cutoff slug date had its rows
  # and gate linkages checked while its decisions log went unchecked.
  ...
  # The exempt line is an escape ONLY when it is the section's SOLE content
  # (PD1) -- conductor_lines counts every OTHER non-blank line in the section
  # (the exempt line itself never reaches that counter; see the "next" in the
  # rule above), so conductor_lines > 0 alongside a set conductor_exempt
  # means real content (a table, orphan text) coexists with it. Treat the
  # exempt line as if absent in that case: it stops suppressing everything
  # and normal row/gate-linkage checks proceed.
  ...
  # ---- Check L: shares this same PD1 adoption gate as Check K, not a
  ...
  # ---- decision-trigger -----------------------------------------------------
  # The union, not the date. Reaching this line at all already means the file
  # is enforced -- the pre-cutoff-without-header case exited above -- so this
  # test is belt-and-braces, and it is written as the union so a future reader
  # does not reinstate the date-only reading.
  if (is_post_adoption) {
    for (d in d_seen) {
...
lint_conductor() {
  ...
  # The legacy prefixless flat glob belongs here for the same reason it
  ...
  # inside the awk. Omitting it (as J deliberately does, for a different
  # reason stated at its own site) made a post-adoption flat file invisible.
  for f in "$PLANS"/active/*.state.md "$PLANS"/finished/*.state.md \
    ...
    # The legacy prefix convention puts active-/finished- BEFORE the date, so
    # reading the date off the raw slug always failed and classified every
    # prefix-legacy file as pre-adoption (0000-00-00) — the same vacuous pass
    # the flat glob hole produced, one layout over.
    slug_date_src="$slug"
...
lint_root() {
  ...
  # Word-splitting an unquoted $(find ...) broke on any path containing a
  ...
  # --- Check I (mozart-orchestration's missing-12b) is deliberately not
  # ported here — see the header comment (DR-5 / DR-8): codex has no stage
  # 12b to gate.

  # --- Check J: DELIVER campaigns missing the 2b. Constraints row -----------
  # active/ ONLY - never finished/, never the bare-slug glob. A finished
  # campaign's stage list is a record of what ran, not a template to conform
  # to.
  #
  # PD20: a bare '3.' row is not evidence of a DELIVER campaign on its own --
  # OPERATE's stage 3 is "Change plan" and INCIDENT's is "Converge", neither
  # of which has a 2b. Fires only when flow_family_of returns DELIVER, or
  # returns empty (no parseable Flow field at all) AND the file has stage
  # rows 2., 3. and 12. -- the shape of a DELIVER stage list with no Flow
  # header to classify it by.
  for f in "$PLANS"/active/*.state.md "$PLANS"/active-*.state.md; do
    [ -f "$f" ] || continue
