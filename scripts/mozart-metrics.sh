...
#
# Covers both artifact roots — .mozart/ (mozart-orchestration's convention;
# a target repo may follow it even under this port) and thoughts/shared/
# (this port's native layout) — and within each, the current subdir
# convention (plans/active/, plans/finished/) plus the legacy prefix +
# prefixless flat layouts.

set -u

ROOT="${1:-.}"

# Roots and the file list are ARRAYS. They used to be whitespace-delimited
# strings, so a repo root containing a space split into two nonexistent roots
# ("no state files"), and a state FILENAME containing a space was handed to
# awk as two truncated paths it then failed to open. Neither is hypothetical
# -- "~/Google Drive/..." and "~/My Repos/..." are ordinary macOS checkout
# locations.
ROOTS=()
...
if [ "${#ROOTS[@]}" -eq 0 ]; then
  echo "mozart-metrics: no $ROOT/.mozart/plans (or $ROOT/thoughts/shared/plans) — nothing to aggregate"
  exit 2
fi
...
# Identical rule to scripts/mozart-lint.sh: honour `\|` as an escaped pipe,
# strip a trailing delimiter so trailing-pipe style does not change the count,
# and let the caller reject a row whose width does not match its header.
# Splitting on a raw pipe made a `source` cell holding a shell pipeline shift
# every later cell, so an empty `control` was counted as controlled -- the
# metric read higher than the evidence supported.
...
# The Tier field is not always the whole line -- a combined header
...
section == "## Conductor record" && /^\|/ {
  ...
  # A shifted row addresses the wrong columns. Do not tally it -- but do not
  # drop it silently either: the count is printed in the conductor section so a
  # corpus that stopped being countable says so instead of reading low.
