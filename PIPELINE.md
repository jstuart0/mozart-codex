...
| **otto** | Infra / k8s / ops reviewer (+ OPERATE change-plan author) | gpt-5.4 |
| **nina** | Cloud specialist — resolves provider-behaviour assertions against a current source | gpt-5.4 |
| **hank** | Ops executor — applies changes to live infrastructure (OPERATE) | gpt-5.4 |
...
4. Pre-flight  — hank runs dry-runs + takes snapshots (records them BEFORE applying);
                 HEAVY adds xander (security surface) + otto (immutable-field/server-dry-run) + claude on the plan
...
**Tiers:** TINY (single reversible change — full loop, but skip otto's separate plan + xander/claude gate) / STANDARD (default) / HEAVY (storage, access control at any layer — Kubernetes RBAC, cloud IAM, identity federation, org structure — secrets, live DB schema, production-stateful, resource-recreation — full pre-flight gate + user sign-off on irreversible steps).
